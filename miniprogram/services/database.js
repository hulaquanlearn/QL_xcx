...
class Query {
  ...
  add({ data }) { if (this.name === 'avatars') return api.updateAvatars(clean(data)).then(() => ({ _id: 'avatars' })); return api.create(this.name, clean(data)).then(result => ({ _id: result._id })); }
  update({ data }) { if (this.name === 'avatars') return api.updateAvatars(clean(data)).then(() => ({ stats: { updated: 1 } })); return api.update(this.name, this.id, clean(data)).then(() => ({ stats: { updated: 1 } })); }
