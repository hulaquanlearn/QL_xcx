...
CREATE TABLE IF NOT EXISTS albums (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (couple_id) REFERENCES couples(id) ON DELETE CASCADE, FOREIGN KEY (author_id) REFERENCES users(id) ON DELETE CASCADE, INDEX idx_albums (couple_id,created_at)
) ENGINE=InnoDB;
CREATE TABLE IF NOT EXISTS avatars (
  couple_id BIGINT UNSIGNED PRIMARY KEY, male_url VARCHAR(1000) NULL, female_url VARCHAR(1000) NULL, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS menus (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (couple_id) REFERENCES couples(id) ON DELETE CASCADE, FOREIGN KEY (author_id) REFERENCES users(id) ON DELETE CASCADE, INDEX idx_menus (couple_id,created_at)
) ENGINE=InnoDB;
CREATE TABLE IF NOT EXISTS orders (
  id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, couple_id BIGINT UNSIGNED NOT NULL, author_id BIGINT UNSIGNED NOT NULL,
  dishes JSON NOT NULL, menu_names JSON NULL, meal_type ENUM('breakfast','lunch','dinner','snack') NOT NULL DEFAULT 'lunch', order_by VARCHAR(40) NOT NULL DEFAULT '', note VARCHAR(500) NOT NULL DEFAULT '', status ENUM('pending','accepted','completed') NOT NULL DEFAULT 'pending', accepted_by VARCHAR(40) NULL, accepted_at DATETIME NULL, completed_at DATETIME NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (couple_id) REFERENCES couples(id) ON DELETE CASCADE, FOREIGN KEY (author_id) REFERENCES users(id) ON DELETE CASCADE, INDEX idx_orders (couple_id,status,created_at)
) ENGINE=InnoDB;
